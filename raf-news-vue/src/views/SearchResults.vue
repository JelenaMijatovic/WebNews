...
export default {
  ...
  methods: {
    ...
    onClickPreviousPage() {
      ...
      this.$axios.get(`/api/articles/search/${this.$route.params.query}/page/${this.currentPage}`).then((response) => {
  ...
  created() {
    this.$axios.get(`/api/articles/search/${this.$route.params.query}/page/1`).then((response) => {
      this.articles = response.data;
      this.onLastPage = this.articles.length < 5;
    });
  },
