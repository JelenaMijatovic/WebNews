...
export default {
  ...
  methods: {
    ...
    onClickPreviousPage() {
      ...
      this.$axios.get(`/api/articles/${this.$route.params.title}/page/${this.currentPage}`).then((response) => {
  ...
  created() {
    this.$axios.get(`/api/articles/${this.$route.params.title}/page/1`).then((response) => {
      this.articles = response.data;
      this.onLastPage = this.articles.length < 5;
    });
  },
